...
export const AddFriendModal = ({ setIsAddFriendModalOpen }: Props) => {
  ...
  return (
    <section className="add-friend-modal">
      <form ref={modalRef}>
        <p>invite friends to {loggedinUser?.username}'s server</p>
        <div className="field">
          <label htmlFor="name">
            Friend Username<span className="red">*</span>
          </label>
          <input
            type="text"
            id="name"
          ...
          {users &&
            users?.map((user) => {
              return (
                <li key={user._id}>
                  <p>{user.username}</p>
                  <button className="btn1" onClick={(ev) => handleInvite(ev, user._id)}>
                    INVITE
                  </button>
                </li>
              )
