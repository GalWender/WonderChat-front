import { EditUser, MiniUser, User } from "../../interfaces/user"
import { UserActionType } from "../../interfaces/actions"
...
export const login = (creds: { email: string, password: string }) => {
    return async (dispatch: Dispatch<UserAction>) => {
        try {
            ...
            const savedUser: MiniUser = {
                _id: loggedinUser.attributes.sub,
                ...
                phone: loggedinUser.attributes.phone_number,
                code: loggedinUser.attributes['custom:code'],
                group: loggedinUser.attributes['custom:group'],
                favorite: JSON.parse(loggedinUser.attributes['custom:favorite']),
                profilePic:loggedinUser.attributes.picture,
                location: loggedinUser.attributes['custom:location'],
                createdAt: +loggedinUser.attributes['custom:createdAt']
            }
            if (loggedinUser.attributes['custom:position']) {
                savedUser.position = loggedinUser.attributes['custom:position']
            }
            userService.saveLocalUser(savedUser)
            return dispatch({ type: UserActionType.SET_USER, payload: savedUser })
        }
        catch (err) {
            throw new Error()
        }
